...
import { Loader2, X, ChevronLeft, CreditCard, ShieldCheck, Building2, Layers } from 'lucide-react';
...
const ResultsTable = ({ searchTerm = "", isAuthenticated }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const url = `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/data!A1:AZ5000?key=${apiKey}`;
      try {
 ...
 const getCol = (name) => {
    ...
    return data.headers.findIndex(h => h && h.trim().toLowerCase() === name.trim().toLowerCase());
  };
  const idx = {
  ...
  // --- תיקון החיפוש: בודק את כל השדות הרלוונטיים ---
  const filteredRows = data.rows.filter(row => {
    ...
    const category = row[idx.category]?.toString().toLowerCase() || "";

    return name.includes(s) || doctor.includes(s) || code.includes(s) || category.includes(s);
  });
    ...
    <div className="max-w-5xl mx-auto px-4 py-6" dir="rtl">
      <div className="flex flex-col gap-3">
        {filteredRows.map((row, index) => (
          <div key={index} onClick={() => setSelectedRow(row)} className="bg-white border border-slate-100 p-4 rounded-2xl shadow-sm hover:border-[#D4AF37] cursor-pointer flex items-center justify-between group transition-all">
            ...
            <div className="flex items-center gap-3">
              ...
              <ChevronLeft size={18} className="text-slate-300 group-hover:text-[#D4AF37] transition-all" />
            </div>
          </div>
        ))}
      </div>
      ...
      {selectedRow && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-lg rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <button onClick={() => setSelectedRow(null)} className="absolute top-5 left-5 p-2 bg-slate-100 hover:bg-red-50 rounded-full z-10 transition-colors">
              <X size={20} />
            </button>
            ...
            <div className="p-6 space-y-6">
              ...
              {/* --- תיקון הקופות: הצגת כל הקופות מחדש --- */}
              <div>
